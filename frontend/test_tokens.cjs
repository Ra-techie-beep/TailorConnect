require("dotenv").config({ path: "./server/.env" }); // Load from server dir
...
async function testTokens() {
    ...
    try {
        ...
        if (error.response && error.response.status === 404) {
             console.log("✅ Passed: Token accepted block. (User not found in DB is perfectly fine for this test email).");
